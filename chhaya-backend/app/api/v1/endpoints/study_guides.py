...
def create_study_guide(
    ...
    try:
        return study_guide_service.create_and_generate(db, user_id=current_user.id, payload=payload)
    except NotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))


@router.get("", response_model=list[StudyGuideOut])
def list_study_guides(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_study_guide(
    guide_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    try:
        return study_guide_service.get_guide_for_user(db, user_id=current_user.id, guide_id=guide_id)
    except NotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
