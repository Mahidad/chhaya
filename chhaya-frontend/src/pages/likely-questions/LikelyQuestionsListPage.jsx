...
import {
  ...
  listLikelyQuestionSets,
} from "../../api/likelyQuestions";

const STATUS = {
  pending: { label: "Pending" },
  analyzing: { label: "Analysing", variant: "amber" },
...
export default function LikelyQuestionsListPage() {
  ...
  useEffect(() => {
    listLikelyQuestionSets().then(setSets).catch(() => setSets([]));
  }, []);
  ...
  return (
    <AppShell section="Likely questions" current="All predictions">
      <div className="page-head">
        <div>
          <div className="page-title">Likely exam questions</div>
          <div className="page-sub">
            Generate study-practice predictions from the patterns in your past papers.
          </div>
        </div>
        <Button
          icon={<Icon name="plus" size={16} />}
          onClick={() => navigate("/likely-questions/new")}
        >
          Generate questions
        </Button>
      </div>
      ...
      {sets.length === 0 ? (
        <div className="ingest-empty">
          ...
          <div className="empty-copy">
            Choose ready OCR papers and Gemini will analyse their exam patterns.
          </div>
          <div className="empty-actions">
            <Button onClick={() => navigate("/likely-questions/new")}>
          ...
          {sets.map((item) => {
            ...
            return (
              <div key={item.id} style={{ display: "flex", alignItems: "center" }}>
                <Link
                  to={`/likely-questions/${item.id}`}
                  className="prow"
                  style={{ flex: 1, textDecoration: "none", color: "inherit" }}
                >
                  <div className="avatar av-ink">
                    <Icon name="fileText" size={15} />
                  </div>
                  <div className="prow-id" style={{ flex: 1 }}>
                    <div className="prow-name">{item.title}</div>
                    <div className="prow-course">
                      {item.source_paper_count} source paper
                      {item.source_paper_count === 1 ? "" : "s"}
                ...
                <button
                  type="button"
                  title="Delete likely-question set"
